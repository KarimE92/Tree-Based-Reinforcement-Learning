...
NUM_SPLIT_EPISODES = 200 #Number of splitting episodes
NUM_TRAIN_EPISODES = 200 #Number of training episodes
NUM_TEST_EPISODES = 250 #Number of testing episodes
...
STOPPING_CRITERIA = 0.0000000000001 #Since values are very small we must have a small stopping criteria for our splits
...
def split():
    ...
    for episodes in range(1,NUM_SPLIT_EPISODES+1): #for each episode
        ...
        for timestep in range(0, MAX_TRAIN_TIME):#for each timestep
            ...
            reward = -100*(abs(obv[2]) - abs(prevobv[2])) #calculate the new reward
            currentnode.SetQ([prevobv, action, obv, reward], learning_rate) #calculate the new QValue for our state
            currentnode.AddObvTuple([prevobv, action, obv, (reward), 0]) #store the transition tuple in our state
            
            if done: #if the cart tips over
...
def train():
    ...
    for episode in range(NUM_TRAIN_EPISODES): #For each episode        
        ...
        for t in range(MAX_TRAIN_TIME): #for each timestep
            ...
            reward = -100*(abs(obv[2]) - abs(prevobv[2])) #calculate new reward function
...
def graph():
    ...
    plt.title("Continuous U-Tree Cart Pole Performance") #label the graph
...
if __name__ == "__main__": #This if statement means that if we import this file into another program it will not automatically run the code    
    ...
    print('Total runtime for U-Tree Cart Pole is: ', stop - start, "seconds")
