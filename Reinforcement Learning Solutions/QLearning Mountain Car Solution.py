...
NUM_BUCKETS = (10, 10)#Take the number of discrete states per observation (to a minimum of 1)
NUM_ACTIONS = env.action_space.n #(take the total number of actions in the environment)
q_table = np.zeros(NUM_BUCKETS + (NUM_ACTIONS,))

# Bounds for each discrete state
STATE_BOUNDS = list(zip(env.observation_space.low, env.observation_space.high))
#Bounds[0] and Bounds[1] must be set manually as the observation space for those two are unbound
STATE_BOUNDS[0] = (-1.2, 0.6)
...
NUM_TRAIN_EPISODES = 500 #Number of training episodes
...
def train():
    ...
    for episode in range(NUM_TRAIN_EPISODES): #For each episode
        ...
        for t in range(MAX_TRAIN_T): #For each timestep
            ...
            reward = 100*((math.sin(3*obv[0]) * 0.0025 + 0.5 * obv[1] * obv[1]) - (math.sin(3*prevobv[0]) * 0.0025 + 0.5 * prevobv[1] * prevobv[1])) #Set new reward using reward function
...
def test():
    ...
    for episode in range(NUM_TEST_EPISODES): #for each episode
        ...
        while not(done): #while the car has not reached the goal
...
def graph():
    ...
    plt.title("QLearning Mountain Car Performance") #label the graph
...
if __name__ == "__main__": #This if statement means that if we import this file into another program it will not automatically run the code    
    ...
    print('Total runtime for QLearning Mountain Car is: ', stop - start, "seconds")
