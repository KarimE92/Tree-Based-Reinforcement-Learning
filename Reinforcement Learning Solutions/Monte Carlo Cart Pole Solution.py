...
#Boudns[1] and Bounds[3] must be set manually as the observation space for those two are unbound
...
def train():
    ...
    for episode in range(NUM_TRAIN_EPISODES): #For each episode
        stateactionlist = [] #initialise the state action list
        ...
        for t in range(MAX_TRAIN_T): #For each timestep
            ...
            stateactionlist.append([state_0, action]) #record the current state and action about to be executed
            obv, reward, done, _, _ = env.step(action) #Execute action
            state_0 = state_to_bucket(obv) #Get our new state
        ...
        time = len(stateactionlist) #since state action pairs are recorded every timstep, the length of the array is the total amount of time
        for sapair in stateactionlist:
            G = 0
            for k in range(0,(time - 1 - (stateactionlist.index(sapair)))):
                G+= pow(discount_factor, k) * 1 #Calculating the return
            if q_table[sapair[0]][sapair[1]] != 0: #If this is not the first return
                q_table[sapair[0]][sapair[1]] = (q_table[sapair[0]][sapair[1]] + G)/2 #Get the average return
            else: #If it is the first return
                q_table[sapair[0]][sapair[1]] = G #The value becomes the return
...
def graph():
    ...
    plt.title("Monte Carlo Cart Pole Performance") #label the graph
...
if __name__ == "__main__": #This if statement means that if we import this file into another program it will not automatically run the code    
    ...
    print('Total runtime for Monte Carlo Cart Pole is: ', stop - start, "seconds")
